...
export default function MyBookingsSCCompletedNDetails() {
    const navigate = useNavigate();
    const {urgentStep, setUrgentStep} = useContext(Context);
    const [ratingModal,setRatingModal] = useState(false);
    useEffect(()=>{setUrgentStep(5)},[]);
  return (
    <div className="">
      <div className="hidden sm:block"><Navbar /></div>
      <div className="flex flex-col bg-[#F4F4F4]/50 sm:px-[120px] px-[20px] pt-[24px]">
        ...
        <div className="sm:flex sm:flex-row flex-col gap-[30px] mt-[8px] pb-[48px]">
          ...
          <div className=" sm:w-[774px] w-[360px] border border-[#EEF0F3] rounded-2xl bg-white">
            ...
            <div className="py-[20px] px-[20px] border-t border-[#EEF0F3]">
                ...
                <Steps stepNo="04" step={urgentStep} mainTitle="Completed" timing="18, June 2023" isLast={true} onClickAction={()=>{setUrgentStep(5)}}>
                    <div></div>
                </Steps>
                {urgentStep > 4 && <button className="flex justify-center items-center text-white text-[14px] font-[600] leading-[116%] bg-[#41B079] w-[300px] h-[36px] py-[24px] rounded-[8px]" onClick={()=>{setRatingModal(true)}}>Add Feedback</button>}
                {ratingModal && <RatingModal onClose={()=>{setRatingModal(false)}} />}
            </div>
          </div>
