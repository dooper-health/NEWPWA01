...
export default function MyBookingsUCDetails() {
    const { id } = useParams(); 
    ...
    useEffect(() => {
      const fetchBookingDetails = async () => {
          try {
              const response = await axios.get(`/api/urgentbooking/urgent-cases/${id}`);
    ...
    <div className="">
      ...
      {videoCallPage ? <VideoCall onCallEnd={()=>{setVideoCallPage(false)}}/> :<div className="flex flex-col bg-[#F4F4F4]/50 sm:px-[120px] px-[15px] pt-[24px]">
        ...
        <div className="sm:flex sm:flex-row flex-col gap-[30px] mt-[8px] pb-[48px]">
          ...
          <div className="sm:w-[774px] w-full sm:border sm:border-[#EEF0F3] rounded-2xl sm:bg-white">
            ...
            <div className="h-[84px] py-[20px] px-[20px] border-[#EEF0F3]">
                ...
                <h2 className="flex font-Montserrat text-[12px] font-medium mt-[8px] w-[345px]">
                    ...
                    />
                    {new Date(bookingDetails.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
                </h2>
            ...
            <div className="py-[20px] px-[20px] border-t border-[#EEF0F3]">
                  <Steps stepNo="01" step={urgentStep} mainTitle="Created" timing={`${new Date(bookingDetails.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })} | ${new Date(bookingDetails.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`} >
                    <div className="flex flex-col gap-[20px]">
                        <div className="w-fit w-[120px] sm:mx-0 -ml-[70px]">
                ...
                <Steps stepNo="02" step={urgentStep} mainTitle="Video Consultation" timing="18, June 2023" action={()=>{setUrgentStep(2)}} >
                    <div className="flex flex-col gap-[20px]">
                        <p>Join video call with doctor.</p>
                        <div className="cursor-pointer flex gap-[8px] items-center bg-[#41B079] w-fit px-[24px]  py-[10px] rounded-[8px]" onClick={()=>{setUrgentStep(3); setVideoCallPage(true)}}>
                            <img src={linkIcon} alt="link" />
                            <p className="text-white text-[14px] font-[600] leading-[116%]">Join</p>
                        </div>
                    </div>
                </Steps>
                <Steps stepNo="03" step={urgentStep} mainTitle="Prescription" timing="18, June 2023" >
                    <div className="sm:flex sm:flex-row sm:justify-between flex-col items-center gap-[8px] w-full pb-[16px] sm:space-y-0 space-y-2">
                      <h1 className="text-[14px] font-[600] leading-[116%]">Doctor's Prescription</h1>
                      <div className="flex gap-[8px] items-center sm:mx-0 -ml-[75px]">
                ...
                <Steps stepNo="04" step={urgentStep} mainTitle="Lab Test" timing="18, June 2023" >
                <div className="sm:flex sm:flex-row sm:justify-between flex-col items-center gap-[8px] w-full pb-[16px] sm:space-y-0 space-y-2">
                ...
                {openLabTest && <LabTestModal onClose={setOpenLabTest} action={()=>{setUrgentStep(5)}}/>}
                <Steps stepNo="05" step={urgentStep} mainTitle="Attendee Assigned" timing="18, June 2023" onClickAction={()=>{setUrgentStep(5)}} >
                ...
                <Steps stepNo="06" step={urgentStep} mainTitle="Attendee Service Started" timing="18, June 2023" onClickAction={()=>{setUrgentStep(6)}}>
                    <div></div>
                </Steps>
                <Steps stepNo="07" step={urgentStep} mainTitle="Lab Report" timing="18, June 2023" onClickAction={()=>{setUrgentStep(7)}} >
                    <div className="flex sm:justify-end w-full">
                      <div className="w-fit w-[120px] sm:mx-0 -ml-[60px]">
                        <PrimaryButton title="Download Report" action={()=>{setUrgentStep(8)}} />
                      </div>
                    </div>
                </Steps>
                <Steps stepNo="08" step={urgentStep} mainTitle="Follow up" timing="18, June 2023" >
                    <div className="flex flex-col gap-[10px]">
                      <p className="text-[#5B6572] text-[14px] font-[500] leading-[116%]">You can follow up with the doctor for 3 Days</p>
                      <button className="flex justify-center items-center text-white text-[14px] font-[600] leading-[116%] bg-[#41B079] sm:w-[300px] h-[36px] py-[24px] rounded-[8px]" onClick={()=>{setUrgentStep(9)}}>Follow Up</button>
                    </div>
                </Steps>
                <Steps stepNo="09" step={urgentStep} mainTitle="Completed" timing="18, June 2023" isLast={true} onClickAction={()=>{setUrgentStep(10)}}>
                    <div></div>
                </Steps>
                {urgentStep > 9 && <button className="flex justify-center items-center text-white text-[14px] font-[600] leading-[116%] bg-[#41B079] w-[300px] h-[36px] py-[24px] rounded-[8px]" onClick={()=>{setRatingModal(true)}}>Add Feedback</button>}
