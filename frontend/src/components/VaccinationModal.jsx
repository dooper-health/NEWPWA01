...
//   const validateForm = () => {
//     const newErrors = {};
//     //  if(!formData.Lab.trim()) newErrors.Lab = 'Vaccination Name is required'
...
//     try {
//       await axios.post('/api/sd/vaccine', data);
...
//               <label className='text-[#8D98A4] text-xs w-full font-Montserrat'>Enter  Vaccination</label>
...
//                 <p className="text-[14px] font-[500] font-Montserrat">Rs. 399</p>
//               </div>
//             </div> 
//             <div className="w-[221px] h-[120px] flex flex-col sm:justify-end mt-10">
//               <p className="text-[#E40443] text-[12px] text-400  ">
//                 Vaccination price to be sent as Quotation after pharmacy accepts. DRA Charge is Mandatory.
//               </p>
...
import axios from 'axios';
import Context from '../context/AppContext';
import { vaccineData } from '../components/vaccineData'; // Assuming this is the correct path to your data file
...
export default function LabModal(props) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'Lab') {
      const filtered = vaccineData.filter(lab => lab.name.toLowerCase().includes(value.toLowerCase()));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/api/sd/vaccine', data);
            await axios.post('/api/notifications/pwa-to-pharmacy/vaccination', { patientMobile: mobileNumber });
      
      // alert('Report uploaded successfully');
      toast.success('Vaccination request sent successfully');
      navigate('/bookings');
      props.onClose(false);
    } catch (error) {
      console.error('Error uploading report:', error);
  ...
  const timeslots = [
    ...
    "8:00 PM - 9:00 PM", "9:00 PM - 10:00 PM", "10:00 PM - 11:00 PM", "11:00 PM - 12:00 AM",
    "12:00 AM - 1:00 AM", "1:00 AM - 2:00 AM", "2:00 AM - 3:00 AM", "3:00 AM - 4:00 AM",
    "4:00 AM - 5:00 AM", "5:00 AM - 6:00 AM", "6:00 AM - 7:00 AM"
  ];
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-50">
    <div className="fixed top-0 left-0 right-0 bottom-0 bg-black/30" onClick={() => props.onClose(false)}></div>
