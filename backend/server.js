...
import urgentCaseRoutes from './routes/urgentCaseRoutes.js';
import StandaloneServiceRoutes from './routes/StandaloneServiceRoutes.js';
...
app.use(cors({origin: ['https://user.dooper.in']}));
...
const storage = new GridFsStorage({
  url: 'mongodb+srv://pwa:pwa1008jaishreeram@pwacluster.92opuxc.mongodb.net/?retryWrites=true&w=majority&appName=pwaCluster', // Update with your MongoDB URI
  file: (req, file) => {
...
app.post('/api/signup1', upload.single('profilePhoto'), async (req, res) => {
  try {
    const { fullName, addressLine1, pincode,city, state, email, emergencyContactNumber, mobileNumber } = req.body;
